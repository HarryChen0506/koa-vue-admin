...
module.exports = (app) => {
  ...
  const RoleSchema = new Schema({
    ...
    delete: { type: Number, min: 0, max: 1, default: 0 }, // 是否删除
  ...
  // 'roles' 数据库集合名，不加的话会默认model名的复数形式
  // Role:模型名（可不要） roles-数据库里集合名（必需）
  // Mongoose会将集合名称设置为模型名称的小写版。
  // 如果名称的最后一个字符是字母，则会变成复数；
  // 如果名称的最后一个字符是数字，则不变；
  // 如果模型名称为"MyModel"，则集合名称为"mymodels"；
  // 如果模型名称为"Model1"，则集合名称为"model1"
  roleModel = roleModel || mongoose.model('Role', RoleSchema, 'roles')
  return roleModel
}
